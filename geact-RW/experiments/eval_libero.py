...
def _resolve_env(obj):
    """Resolve ``${oc.env:VAR}`` / ``${oc.env:VAR,default}`` placeholders via os.environ.

    The training entry loads configs through OmegaConf, but this eval loader uses
    plain ``yaml.load``; this makes the same env-var-portable configs work here too.
    """
    import re
    pat = re.compile(r"\$\{oc\.env:([^},]+)(?:,([^}]*))?\}")

    def repl(m):
        var, default = m.group(1).strip(), m.group(2)
        if var in os.environ:
            return os.environ[var]
        if default is not None:
            return default
        raise KeyError(
            f"Config references env var '{var}' but it is not set and has no default. "
