...
from models.action_patches.patches import preprocessing_action_states, add_action_expert, prepend_align_prefix
...
class LTXVideoTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin, PeftAdapterMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        align_prefix_len = 0
        action_self_attention_mask = None
        if return_action:
            ...
            ### future-DINO alignment: prepend K prefix tokens (bidirectional;
            ### env kill-switch GEACT_ALIGN_DISABLE=1 forces the stock path for A/B checks)
            if getattr(self, "align_enabled", False) and not os.environ.get("GEACT_ALIGN_DISABLE"):
                n_history = 1 if history_action_state is not None else 0
                action_hidden_states, action_temb, action_rotary_emb, action_self_attention_mask = prepend_align_prefix(
                    self,
                ...
                align_prefix_len = self.align_num_prefix_tokens
                if not self.training and not getattr(self, "_align_eval_logged", False):
                    ### one-time confirmation that the prefix is active in the
                    ### eval/inference forward (required for the bidirectional
                    ### variant, where the prefix must be present at inference)
                    print(
                        f"[future-align] eval forward: prefix_len={align_prefix_len}",
                        flush=True,
                    )
                    self._align_eval_logged = True
            ...
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                if return_action:
                    ...
                    action_hidden_states = self.action_blocks[block_idx](
                        ...
                        self_attention_mask=action_self_attention_mask,
                    )
        ...
        if return_action:
            if align_prefix_len > 0:
                ### split prefix hidden states off BEFORE the action output head:
                ### the head only ever sees action-token hidden states.
                align_prefix_hidden = action_hidden_states[:, :align_prefix_len]
                action_hidden_states = action_hidden_states[:, align_prefix_len:]
                final_output['align'] = self.align_proj(align_prefix_hidden)
