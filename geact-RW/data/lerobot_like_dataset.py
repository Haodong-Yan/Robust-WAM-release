...
class CustomLeRobotDataset(Dataset):
    def __init__(self,
        ...
        fix_mem_idx = None,
        stat_file = None,
        align_dino_targets = False,
        align_num_future_frames = 8,
        align_target_dim = 768,
        dino_target_tag = "dino_targets_perframe",
    ):
        """
        ...
        ### future-DINO alignment targets: per-frame DINOv3 CLS features stored
        ### alongside the dataset as <domain>/<dino_target_tag>/chunk-XXX/episode_XXXXXX.pth
        ### -> {"dino_target": float16 [T, V, hid]} (V = cameras, same order as valid_cam).
        ### At sample time we gather the CLS rows of the 8 future video frames the
        ### model predicts (action window offsets stride-1::stride) x V cameras,
        ### frame-major -> [K, hid], plus a validity flag for loss masking.
        self.align_dino_targets = bool(align_dino_targets)
        self.align_num_future_frames = int(align_num_future_frames)
        self.align_target_dim = int(align_target_dim)
        self.dino_target_tag = dino_target_tag

    def get_dino_target(self, parquet_path, indexes):
        """Gather frame-major [K, hid] DINO CLS targets for the sampled window."""
        num_cams = len(self.valid_cam)
        K = self.align_num_future_frames * num_cams
        try:
            dino_path = parquet_path.replace(
                f"{os.sep}data{os.sep}chunk-", f"{os.sep}{self.dino_target_tag}{os.sep}chunk-"
            ).replace(".parquet", ".pth")
            pack = torch.load(dino_path, map_location="cpu", weights_only=False)
            per_frame = pack["dino_target"]  # [T, V, hid] float16
            assert per_frame.ndim == 3 and per_frame.shape[1] == num_cams, per_frame.shape
            action_window = indexes[-self.action_chunk:]
            fut_fids = action_window[self.video_temporal_stride - 1::self.video_temporal_stride]
            fut_fids = fut_fids[:self.align_num_future_frames]
            fid = torch.as_tensor(
                [min(int(f), per_frame.shape[0] - 1) for f in fut_fids], dtype=torch.long
            )
            target = per_frame[fid]                                   # [T_f, V, hid]
            target = target.reshape(-1, per_frame.shape[-1]).float()  # frame-major [K, hid]
            assert target.shape[0] == K, (target.shape, K)
            return target, torch.tensor(1.0)
        except Exception:
            return torch.zeros(K, self.align_target_dim), torch.tensor(0.0)
    ...
    def get_batch(self, idx):
        ...
        extra = {}
        if self.align_dino_targets:
            dino_target, dino_valid = self.get_dino_target(parquet_path, indexes)
            extra['dino_target'] = dino_target
            extra['dino_valid'] = dino_valid

        return videos, action, caption, state, extra
    ...
    def __getitem__(self, idx):        
        ...
        if self.fix_epiidx is not None:
            video, actions, caption, state, extra = self.get_batch(self.fix_epiidx)
        else:
            while True:
                try:
                    video, actions, caption, state, extra = self.get_batch(idx)
                    break
                except:
