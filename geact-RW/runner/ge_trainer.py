...
class Trainer:
    ...
    def __init__(self, config_file, to_log=True, output_dir=None) -> None:
        ...
        ### future-DINO alignment (config block `future_alignment:`; absent -> disabled)
        _fa = getattr(self.args, "future_alignment", None) or {}
        self.align_enabled = bool(_fa.get("enabled", False))
        self.align_loss_weight = float(_fa.get("loss_weight", 0.05))
        self.align_warmup_steps = int(_fa.get("warmup_steps", 5000))
        ...
        for epoch in range(first_epoch, self.state.train_epochs):
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                with accelerator.accumulate([ self.diffusion_model ]):
                    ...
                    ### future-DINO alignment loss: per-token cosine between the K
                    ### projected prefix hidden states and frozen DINO CLS targets.
                    ### Hard warmup gate (FastWAM v6): weight 0 before warmup_steps —
                    ### the 0-scaled term keeps align params in the autograd graph so
                    ### DeepSpeed sees gradients for every trainable parameter.
                    loss_align = None
                    if self.align_enabled and self.args.return_action and 'align' in pred_all:
                        align_pred = pred_all['align']  # [B, K, hid]
                        align_target = batch['dino_target'].to(accelerator.device, dtype=torch.float32)
                        align_valid = batch['dino_valid'].to(accelerator.device, dtype=torch.float32)  # [B]
                        cos = torch.nn.functional.cosine_similarity(
                            align_pred.float(), align_target, dim=-1
                        )  # [B, K]
                        per_sample = (1.0 - cos).mean(dim=1)  # [B]
                        denom = align_valid.sum().clamp(min=1.0)
                        loss_align = (per_sample * align_valid).sum() / denom
                        align_weight_eff = self.align_loss_weight if global_step >= self.align_warmup_steps else 0.0
                        loss = loss + align_weight_eff * loss_align
                ...
                if loss_align is not None:
                    loss_align = accelerator.reduce(loss_align.detach(), reduction='mean')
                    logs["loss_align"] = loss_align.item()
                ...
                if global_step % self.args.steps_to_log == 0:
                    if accelerator.is_main_process:
                        if self.writer is not None:
                            ...
                            if loss_align is not None:
                                self.writer.add_scalar("Align loss", loss_align.item(), global_step)
                                self.writer.add_scalar(
                                    "Align loss weighted",
                                    (self.align_loss_weight if global_step >= self.align_warmup_steps else 0.0) * loss_align.item(),
                                    global_step,
                                )
            ...
            ### the in-loop break above only exits the dataloader loop; without this
            ### guard every subsequent epoch trains ONE more step then breaks again
            ### (and the final save below never runs). Exit the epoch loop too.
            if global_step >= self.state.train_steps:
                logger.info(">>> max train step reached — stopping training")
                break
